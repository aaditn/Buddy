...
//    private static let relayCharacteristicUuid = "0xFF40"
    ...
    private static let sEdge: ASPickerDisplayItem = {
        let descriptor = ASDiscoveryDescriptor()
        descriptor.bluetoothServiceUUID = AccessoryModel.buddy.serviceUUID

        return ASPickerDisplayItem(
            name: AccessoryModel.buddy.displayName,
            productImage: AccessoryModel.buddy.accessoryImage,
    ...
    func presentPicker() {
        session.showPicker(for: [Self.sEdge]) { error in
            if let error {
    ...
    private func saveAccessory(accessory: ASAccessory) {
        ...
        if accessory.displayName == AccessoryModel.buddy.displayName {
            accesoryModel = .buddy
        }
    }
